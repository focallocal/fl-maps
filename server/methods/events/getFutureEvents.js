    ...
    const events = Events.find(
      {
        ...
        '$or': [{ 'when.endingDate': { $gte: new Date() } }, { 'when.recurring.forever': { $eq: true } }, { 'when.multipleDays': { $eq: true } }, { 'when.recurring.recurrenceEndDate': { $gte: new Date() } }, { $and: [{ 'when.repeat': { $eq: true } }, { 'when.recurring.until': { $gte: new Date() } }] }]
      },
      {
        skip,
        limit: 30
