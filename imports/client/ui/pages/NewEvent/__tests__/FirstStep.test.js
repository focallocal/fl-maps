...
describe('<FirstStep />', () => {
  ...
  it('should render', () => {
    expect(wrapper.find('#first-step').exists()).toBe(true)
  })
  ...
  it('should hide hours fields if multipleDays is checked', () => {
    ...
    expect(wrapper_.find('#first-step .dates-hours').find(AutoField)).toHaveLength(4)
    multipleDays.at(2).simulate('click')
    expect(wrapper_.find('#first-step .dates-hours').find(AutoField)).toHaveLength(2)
  })
